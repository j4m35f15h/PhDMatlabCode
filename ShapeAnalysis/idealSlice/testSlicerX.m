...
sliceDepths = [min(composite.vertices(:,1)),max(composite.vertices(:,1))];
...
sliceResScale = [1.6*ones(1,33),2.2*ones(1,33),1.6*ones(1,34)];
...
for i = 1:size(sliceDepths,2)
    ...
    for j = 1:size(composite.vindex,2)-1
        ...
        temp = find(abs(composite.vertices(organSel,1)-sliceDepths(i))<(sliceRes/sliceResScale(i)));
...
%         figure
%         hold on
%         scatter(composite.vertices(organSel(temp),2),composite.vertices(organSel(temp),3))
%         %                 scatter(edgesX(xMax),edgesY(yMax))
%         k = boundary(composite.vertices(organSel(temp),2),composite.vertices(organSel(temp),3),0.2);
%         plot(composite.vertices(organSel(temp(k)),2),composite.vertices(organSel(temp(k)),3))
% %         k = boundary(composite.vertices(organSel(temp(region2)),2),composite.vertices(organSel(temp(region2)),3),0.4);
% %         plot(composite.vertices(organSel(temp(region2(k))),2),composite.vertices(organSel(temp(region2(k))),3))
%         hold off
        %Conv Hull method
...
%         while 1
%             'ping'
%             outline1 = boundary(composite.vertices(organSel(temp(region1)),1),composite.vertices(organSel(temp(region1)),2),0.3);
%             xcoor = composite.vertices(organSel(temp(region1(outline1))),1);
%             xmean = mean(abs(diff(xcoor)));
%             xdiff = diff(xcoor);
%             xdiffloc = find((abs(xdiff)/xmean)>1);
%             if isempty(xdiffloc)
%                 break
%             end
%             region1(xdiffloc) = [];
%         end
        ...
        outline1 = boundary(composite.vertices(organSel(temp),2),composite.vertices(organSel(temp),3),0.2);
        if ~isempty(outline1)
            [~,I] = max(composite.vertices(organSel(temp(outline1)),3));
            outline1 = circshift(outline1,size(outline1,1)+1-I);
            outline1 = [outline1;outline1(1)];
            [trueOutline(1:1000,1,j),trueOutline(1:1000,2,j)] = evenOut(composite.vertices(organSel(temp(outline1)),2),composite.vertices(organSel(temp(outline1)),3));
        end
...
%         [trueOutline(1001:2000,1,j),trueOutline(1001:2000,2,j)] = evenOut(composite.vertices(organSel(temp(region2(outline2))),1),composite.vertices(organSel(temp(region2(outline2))),2));
    ...
    xlim([min(composite.vertices(:,2)) max(composite.vertices(:,2))])
    ylim([0 max(composite.vertices(:,3))])
    set(f,'Color','black')
